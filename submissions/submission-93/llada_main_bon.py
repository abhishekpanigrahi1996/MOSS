...
try:
    from generate import generate as generateVanillaPRM
    from generate_optimized import generate_prm_window_score as generateBackMasking
...
RESULTS_CSV_PATH = "/my_vol/math_evaluation_results_detailed_n=5-optimcsv"
MAX_QUESTIONS = None
...
def run_evaluation(model, tokenizer, prm_model, prm_tokenizer):
    ...
    for i, (question, ground_truth) in enumerate(zip(questions, answers)):
        ...
        #     print(f"\n--- Vanilla Output ---\n{prm_response.strip()}\n")
        #     print(f"Vanilla Correct (Auto): {current_q_results['prm_correct_auto']}")
        # except Exception as e:
        ...
        df_one = pd.DataFrame([current_q_results])

        # If it's the first example and file doesn't exist, write header
        write_header = i == 0 and not os.path.exists(RESULTS_CSV_PATH)

        try:
            df_one.to_csv(RESULTS_CSV_PATH, mode='a', index=False, header=write_header)
            print(f"Appended result for question {i+1} to CSV.")
        except Exception as e:
            print(f"Error writing to CSV for question {i+1}: {e}")
